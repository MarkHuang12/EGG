...
flowchart TD
  subgraph Fund Module
    direction TB
    User["User Initiates Deposit"]
    Validation["Deposit Validation"]
    Fund_Reserve_Account["Fund Reserve Account\n(Primary Storage of Deposited Assets)"]
    User_Fund_Account["User Fund Account\n(User-specific Deposit Tracking)"]
    User_Receipt_Token_Account["User Receipt Token Account\n(Issued Receipt Tokens for Deposits)"]
    Reward_Account["Reward Account\n(Triggers Reward Mechanism)"]
  end

  User --> Validation
  Validation -->|Valid| Fund_Reserve_Account
  Validation -->|Invalid| Reject["Reject & Notify User"]
  Fund_Reserve_Account --> User_Fund_Account
  User_Fund_Account --> User_Receipt_Token_Account
  User_Receipt_Token_Account --> Reward_Account
```
